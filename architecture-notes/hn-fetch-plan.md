...
1. **`deleted: true`**: fetch and store the stub as-is, same as any other item, no special-case code. This resolves the source doc's tombstone-vs-data-loss question as **tombstone**: the row for that id still exists and its payload now shows `deleted: true`, with `by`/`text`/`kids` gone. Bronze overwrites in place on a hash change (`UNIQUE (source, stable_id)`; Arch doc section 4.1 point 4; `ports.py` `RawStorePort.write` docstring), so the previously-captured full item's payload does not survive alongside the tombstone, only the tombstone shape does. That is an accepted, structural consequence of Bronze holding exactly one row per entity, not something worth working around for this one case.
