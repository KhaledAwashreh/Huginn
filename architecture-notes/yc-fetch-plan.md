...
1. Architecture document section 5 already names this as the Phase 0 design ("YC: direct Algolia search-key query" in the adapter diagram, and the adjoining prose). This note keeps that design rather than reopening it; it does not introduce a second decision-maker for the same call. The diagram has since gained a connectors layer beneath the two adapters, and the adapters no longer hold transport, but the decision this note records is unchanged.
...
5. **The `facets` discovery query is confirmed live too (2026-09-09), not just assumed from the plain-search test above.** `browse` being blocked for this key (point 3) meant a facet-counting query (a structurally different Algolia call: `facets: ["batch"], hitsPerPage: 0`) could plausibly have been restricted the same way, and this was never checked before KAN-30 was built against it. Confirmed live: `POST .../query` with `{"query": "", "facets": ["batch"], "hitsPerPage": 0, "maxValuesPerFacet": 1000}` returns HTTP 200 with 50 batch values whose counts sum to exactly 6204, matching the plain query's `nbHits` from point 1 with no gap. `AlgoliaConnector.discover_batches()` in `src/huginn/elt/ingestion/connectors/algolia.py` (extracted from the adapter; the query body asserted by the test is unchanged) is confirmed sound, not an untested assumption.

None of this touches KAN-7 (ToS legal exposure, still open): every live call here (2026-09-08 and 2026-09-09) was a manual, one-off, read-only test made with the user's explicit, informed acceptance of that risk each time, not a resolution of the legal question, and not something to repeat routinely before KAN-7 is actually closed.
