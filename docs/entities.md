...
A second, distinct skip reason declines a hit: an absent or null `launched_at`. `OccurredOn` is the signal's own date and no layer invents a substitute for it, so a listing YC reports no launch date for cannot be dated and is not promoted. This is a source-side data gap rather than a business rule, and the loader reports it under its own count rather than folding it into the excluded one, so a rise in undated listings is visible as a data problem instead of hiding behind a deliberate exclusion. A hit that is both acquired and undated is counted once, as not a prospect, because the status check runs first.

The not-a-prospect bucket is deliberately count-only, with no per-row warning: 1,903 of 6,252 live rows are excluded by design, so naming each one would bury the summary and the genuinely surprising buckets. The undated bucket does name each listing, because a rise in it is a source-side problem worth chasing to a specific YC `id`. A row too malformed to parse is counted as rejected and logged with its `id` and the exception type, since bronze is the as-fetched record and that `id` is the only handle back to the offending payload. A fifth bucket catches a skip reason the summary does not name, so a reason added later cannot be filed silently under a business rule. All five counts account for every bronze row read, so a row lost to a missing bucket shows up as a total that does not add up.

A missing optional field is not a reason to decline a row, only to make it thinner, and the two policies differ on purpose. `Description` is NULL when neither `long_description` nor `one_liner` is present, `""` when both are present and empty, and the `one_liner` fallback otherwise, which is why 386 live rows carrying an empty `long_description` keep their tagline text. NULL and `""` stay distinct for the same reason `Industries` and `FormerNames` keep them distinct.
- Id: GUID
- StableId: String
- CompanyNameRaw: String
