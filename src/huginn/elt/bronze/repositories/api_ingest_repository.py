...
class PostgresApiIngestRepository:
    ...
    Each method opens its own short-lived connection. Callers above this
    boundary hold no connection, no cursor, and no `psycopg` import.
    """
    ...
    def touch(self, source: str, stable_id: str) -> None:
        ...
        with psycopg.connect(self._database_url) as conn, conn.cursor() as cur:
            cur.execute(*build_touch_query(source, stable_id))
